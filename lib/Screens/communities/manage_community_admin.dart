...
class ManageCommunityAdmin extends ConsumerStatefulWidget {
  final Community community;
  const ManageCommunityAdmin({super.key, required this.community});
  @override
  ConsumerState<ManageCommunityAdmin> createState() =>
      _ManageCommunityAdminState();
}

class _ManageCommunityAdminState extends ConsumerState<ManageCommunityAdmin> {
  ...
  Future<void> handleManageAdmin(User user, bool toAdd) async {
    ...
    if (toAdd) {
      widget.community.admin.add(user);
    } else {
      widget.community.admin.removeWhere((admin) => admin.id == user.id);
    }
    map['admin'] = widget.community.admin;
    map['_id'] = widget.community.id;
    updateCommunityApi(context, map);
  }
                ...
                itemBuilder: (context, index) {
                  if (_searchQuery.isEmpty) {
                    ...
                    return PersonItemWidget(
                      user: user,
                      isAdmin: widget.community.admin
                          .any((item) => item.id == _currentUserId),
                      isUserAdmin: widget.community.admin
                          .any((item) => item.id == user.id),
