...
class SelectACouncil extends ConsumerStatefulWidget {
  const SelectACouncil({super.key});
  @override
  ConsumerState<SelectACouncil> createState() => _SelectACouncilState();
}

class _SelectACouncilState extends ConsumerState<SelectACouncil> {
    ...
    return Scaffold(
      ...
      body: festsAsyncValue.when(
        data: (data) {
          return data.isEmpty
              ? const Center(
                  child: Text("You must be admin of atleast one Council"))
              : ListView.builder(
