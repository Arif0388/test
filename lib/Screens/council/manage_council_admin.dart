...
class ManageCouncilAdmin extends ConsumerStatefulWidget {
  final Council council;
  const ManageCouncilAdmin({super.key, required this.council});
  @override
  ConsumerState<ManageCouncilAdmin> createState() => _ManageCouncilAdminState();
}

class _ManageCouncilAdminState extends ConsumerState<ManageCouncilAdmin> {
  ...
  Future<void> handleManageAdmin(User user, bool toAdd) async {
    ...
    if (toAdd) {
      widget.council.admin.add(user);
    } else {
      widget.council.admin.removeWhere((admin) => admin.id == user.id);
    }
    map['admin'] = widget.council.admin;
    map['_id'] = widget.council.id;
    updateCouncilApi(context, map);
  }
                ...
                itemBuilder: (context, index) {
                  if (_searchQuery.isEmpty) {
                    ...
                    return PersonItemWidget(
                      user: user,
                      isAdmin: widget.council.admin
                          .any((item) => item.id == _currentUserId),
                      isUserAdmin:
                          widget.council.admin.any((item) => item.id == user.id),
