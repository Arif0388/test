...
class ManageCollegeAdmin extends ConsumerStatefulWidget {
  final College college;
  const ManageCollegeAdmin({super.key, required this.college});
  @override
  ConsumerState<ManageCollegeAdmin> createState() => _ManageCollegeAdminState();
}

class _ManageCollegeAdminState extends ConsumerState<ManageCollegeAdmin> {
  ...
  Future<void> handleManageAdmin(User user, bool toAdd) async {
    ...
    if (toAdd) {
      widget.college.admin.add(user);
    } else {
      widget.college.admin.removeWhere((admin) => admin.id == user.id);
    }
    map['admin'] = widget.college.admin;
    map['_id'] = widget.college.id;
    updateCollegeApi(context, map);
  }
                ...
                itemBuilder: (context, index) {
                  if (_searchQuery.isEmpty) {
                    ...
                    return PersonItemWidget(
                      user: user,
                      isAdmin: widget.college.admin
                          .any((item) => item.id == _currentUserId),
                      isUserAdmin: widget.college.admin
