...
class ManageFestAdmin extends ConsumerStatefulWidget {
  final Fest fest;
  const ManageFestAdmin({super.key, required this.fest});
  @override
  ConsumerState<ManageFestAdmin> createState() => _ManageFestAdminState();
}

class _ManageFestAdminState extends ConsumerState<ManageFestAdmin> {
  ...
  Future<void> handleManageAdmin(User user, bool toAdd) async {
    ...
    if (toAdd) {
      widget.fest.admin.add(user);
    } else {
      widget.fest.admin.removeWhere((admin) => admin.id == user.id);
    }
    map['admin'] = widget.fest.admin;
    map['_id'] = widget.fest.id;
    updateFestApi(context, map);
  }
                ...
                itemBuilder: (context, index) {
                  if (_searchQuery.isEmpty) {
                    ...
                    return PersonItemWidget(
                      user: user,
                      isAdmin: widget.fest.admin
                          .any((item) => item.id == _currentUserId),
                      isUserAdmin:
                          widget.fest.admin.any((item) => item.id == user.id),
